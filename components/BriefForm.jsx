...
const PRODUCT_TYPES = [
  { key: 'syrup', label: 'Syrup', emoji: '🫙', desc: 'Functional or flavoured syrups' },
  { key: 'other', label: 'Other', emoji: '📦', desc: 'Something else — get in touch' },
]

export default function BriefForm({ brief }) {
  const [productTypeSelected, setProductTypeSelected] = useState(!!brief.formData?.productCategory)
  ...
  const save = async (data) => {
    try {
      await updateDoc(doc(db, 'briefs', brief.id), { formData: data })
    } catch (e) {
    ...
    try {
      ...
      if (brief.productId) {
        ...
        const snap = await getDocs(
          query(collection(db, 'products'), where('briefId', '==', brief.id))
        )
  ...
  const selectProductType = (type) => {
    const updated = { ...formData, productCategory: type }
    setFormData(updated)
    save(updated)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center space-y-4">
          {brief.clientLogoUrl && (
            <img src={brief.clientLogoUrl} alt="" className="h-12 mx-auto object-contain" />
          )}
          <div className="text-5xl">✅</div>
          <h1 className="text-2xl font-bold text-gray-900">Brief received!</h1>
          <p className="text-gray-500">
            Thanks for filling this in. Our team will review everything and be in touch soon.
          </p>
        </div>
      </div>
    )
  ...
  if (!productTypeSelected) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-10 w-full max-w-lg space-y-8">
          <div className="text-center">
            {brief.clientLogoUrl && (
              <img
                src={brief.clientLogoUrl}
                alt=""
            ...
            <h1 className="text-2xl font-bold text-gray-900">Let's build your brief</h1>
            <p className="text-gray-400 text-sm mt-2">
              First — what kind of product are you looking for?
            </p>
          </div>
          ...
          <div className="space-y-3">
            {PRODUCT_TYPES.map((t) => (
              <button
                ...
                className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl border border-gray-200 hover:border-green-400 hover:bg-green-50/50 transition text-left group"
              >
                ...
                <span className="ml-auto text-gray-300 group-hover:text-green-500 transition text-lg">
                  →
                </span>
              </button>
            ))}
        ...
        <div className="max-w-2xl mx-auto px-6 py-4">
          ...
          <div className="flex mt-2 gap-1">
            {STEPS.map((s, i) => (
              <span
                key={s.label}
                className={`text-xs flex-1 text-center truncate transition-colors ${
                  i === step
